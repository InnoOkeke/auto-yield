...
import ManualSavingsModal from './ManualSavingsModal';
...
export default function ManualSavingsCard({ userId, address }: ManualSavingsCardProps) {
    ...
    const handleOpenModal = (mode: 'deposit' | 'withdraw') => {
        setModalMode(mode);
        setShowModal(true);
    };
            ...
            >
                ...
                <div className="grid grid-cols-2 gap-3">
                    <button
                        ...
                        className="flex items-center justify-center gap-2 py-3 px-4 bg-primary text-white rounded-2xl font-bold text-sm hover:opacity-90 transition-all shadow-lg active:scale-95"
                    >
                    ...
                    <button
                        ...
                        className="flex items-center justify-center gap-2 py-3 px-4 bg-foreground/10 text-foreground rounded-2xl font-bold text-sm hover:bg-foreground/20 transition-all active:scale-95"
                    >
                ...
                <Wallet className="absolute -bottom-6 -right-6 w-32 h-32 text-foreground/5 -rotate-12 pointer-events-none" />
            </motion.div>
